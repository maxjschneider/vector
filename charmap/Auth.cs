...
namespace VectorAuth
{
    public class API
    {
        ...
        public bool Login(string username, string password)
        {
            ...
            {
                ShowMessage("Please enter a username and password!", MessageBoxIcon.Error);
            ...
            if (response == null)
            {
                ShowMessage("Invalid server response!", MessageBoxIcon.Error);
            ...
            else
            {
                ...
                try
                {
                    ...
                    switch ((string)json.result)
                    {
                        ...
                        case "paused":
                            {
                                ShowMessage("This application is currently paused!", MessageBoxIcon.Error);
                        ...
                        case "missing credentials":
                            {
                                ShowMessage("Missing credentials.", MessageBoxIcon.Error);

                                return false;
                            }
                        case "banned":
                            {
                                Security.FailFast("Your account is banned.");

                                return false;
                            }
                        case "expired":
                            {
                                ShowMessage("Subscription expired on " + (string)json.expiry + "\n\nContact Dufresne#8634 to extend your subscription.", MessageBoxIcon.Error);

                                return false;
                            }
                        case "invalid hwid":
                            {
                                ShowMessage("Invalid hardware ID.", MessageBoxIcon.Error);

                                return false;
                            }
                        case "invalid credentials":
                            {
                                ShowMessage("Invalid login credentials.", MessageBoxIcon.Error);
        ...
        {
            MessageBox.Show(message, "Vector", MessageBoxButtons.OK, icon);
        }
